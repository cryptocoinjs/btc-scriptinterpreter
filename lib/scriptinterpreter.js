...
var checkSig = ScriptInterpreter.checkSig =
  function(sig, pubkey, scriptCode, tx, n, hashType, callback) {
    ...
    try {
      ...
      // Verify signature
      // TODO: modify ecdsa so that it accepts buffer rather than byte arrays
      signature = binConv(signature, {out: 'bytes'})
      pubkey = binConv(pubkey, {out: 'bytes'})

      var sigResult = ecdsa.verify(hash, signature, pubkey)
      callback(null, sigResult)
    } catch (err) {
      console.log("CheckSig Failed: ", err)
      callback(null, false);
    }
};
